    ...
    <nav className="fixed top-0 w-full bg-white/95 backdrop-blur-sm z-50 shadow-lg border-b border-green-100">
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          ></div>
          <div className="fixed top-20 left-4 right-4 z-50 bg-white rounded-2xl shadow-2xl py-6 px-6 border border-green-100">
            <ul className="flex flex-col gap-4 list-none">
              <li>
                <Link 
                  to="/" 
                  onClick={() => setMenuOpen(false)} 
                  className="block text-lg font-semibold text-green-800 hover:text-green-600 py-2 px-2 rounded-lg hover:bg-green-50 transition-all duration-200"
                >
                  🏠 Inicio
                </Link>
              </li>
              ...
              <li className="pt-4 border-t border-green-100">
                <button 
                  className="w-full bg-green-900 text-white px-5 py-3 rounded-xl flex items-center justify-center gap-3 font-semibold hover:bg-green-800 transition-colors duration-200 shadow-md"
                  onClick={() => {
                    onCartClick();
                    setMenuOpen(false);
                  }}
                >
                  🛒 Ver Carrito 
                  <span className="bg-orange-400 text-green-900 px-2.5 py-0.5 rounded-full text-sm font-bold">
                    {cartCount}
                  </span>
                </button>
              </li>
